...
struct segtree{
    ...
    int get_min(int k, int l, int x, int lx, int rx){
        if (maxs[x] < k || l >= rx)return -1;
        if (rx - lx == 1){
            return lx;
        }
        int m = (lx+rx)/2;
        int v1 = -2;
        if (maxs[2*x+1] >= k)v1 = get_min(k,l, 2*x+1, lx, m);
        if (v1>=0)return v1;
        if (maxs[2*x+2] >= k) return get_min(k,l, 2*x+2, m, rx);
        else return -1;
    }
    int get_min(int k, int l){
        return get_min(k,l, 0, 0, size);
    }
};
int32_t main(){
    ...
    for(int i = 0; i < m; i++){
        ...
        if (t==1) {
            ...
            int val = st.get_min(a, b);
            if (val < n) cout << val << endl;
            else cout << -1 << endl;
        }
    }
    return 0;
